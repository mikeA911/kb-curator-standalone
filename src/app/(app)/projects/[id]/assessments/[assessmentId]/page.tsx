...
import { getAssessmentById, listVersions, getAssessmentMatrixForVersion } from '@/lib/projects/assessments'
import { Markdown } from '@/components/shared/Markdown'
import { AssessmentVersionActions } from '@/components/projects/AssessmentVersionActions'
...
export default async function AssessmentDetailPage({
  ...
  const [{ data: project }, assessment, versions] = await Promise.all([
    supabase.from('projects').select('id, name').eq('id', id).single(),
    getAssessmentById(supabase, assessmentId),
    listVersions(supabase, assessmentId),
  ])
  ...
  if (!targetVersionId) {
    return (
      <div className="flex flex-col gap-4">
        ...
        <p className="text-sm text-zinc-500">This assessment has no version yet.</p>
      </div>
    )
  }

  const matrix = await getAssessmentMatrixForVersion(supabase, assessmentId, targetVersionId)
  if (!matrix) notFound()
  ...
  const canManage = viewerProfile?.role === 'admin' || viewerMembership?.role === 'owner' || viewerMembership?.role === 'curator'
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <div className="mt-2 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              ...
              <span
                className={
                  matrix.version.status === 'active' ? 'text-green-700' : matrix.version.status === 'draft' ? 'text-amber-700' : 'text-zinc-500'
                }
              >
          ...
          {matrix.version.status === 'active' && (
            <Link href={`/projects/${id}/assessments/${assessmentId}/respond`} className="shrink-0 text-sm text-blue-700 underline">
              Submit a response
            </Link>
          )}
        </div>
        ...
        {versions.length > 1 && (
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            {versions.map((v) => (
              <Link
                key={v.id}
                href={`/projects/${id}/assessments/${assessmentId}?version=${v.id}`}
        ...
        {canManage && (
          <AssessmentVersionActions
            projectId={id}
            assessmentId={assessmentId}
            versionId={matrix.version.id}
            status={matrix.version.status}
          />
        )}
      </div>
        ...
        {matrix.questions.map((q, i) => (
          <div key={q.id} className="rounded border border-zinc-200 bg-white p-4">
            ...
            <ul className="mt-3 flex flex-col gap-2 border-t border-zinc-100 pt-3">
              {matrix.responses.length === 0 && <li className="text-sm text-zinc-500">No responses submitted yet.</li>}
                      ...
                      <p className="text-sm text-zinc-500">
                        ...
                        {r.status === 'in_progress' && (
                          <>
                            ...
                            ·{' '}
                            <Link
                              href={`/projects/${id}/assessments/${assessmentId}/respond?participant=${encodeURIComponent(r.participant_label)}`}
