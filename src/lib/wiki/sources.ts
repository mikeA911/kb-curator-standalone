...
export async function linkSource(supabase: SupabaseClient<Database>, input: LinkSourceInput) {
  if (input.sourceType !== 'external' && !input.documentId && !input.chunkId) {
    throw new WikiValidationError('A document or chunk is required unless sourceType is "external"')
  }
...
export async function getSourcesForVersion(supabase: SupabaseClient<Database>, versionId: string) {
  const { data, error } = await supabase
    .from('wiki_sources')
    .select('*, document:documents(id, original_filename, doc_type), chunk:document_chunks(id, chunk_index, source_page, chunk_text)')
