...
// One aggregator over four independently-owned queries -- deliberately not a
...
export async function getNeedsAttention(supabase: SupabaseClient<Database>): Promise<NeedsAttentionItem[]> {
  const [{ data: submittedDocs }, unpublishedArticles, { data: failedRuns }, draftProjects] = await Promise.all([
