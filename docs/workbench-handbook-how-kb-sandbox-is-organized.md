...
|---|---|---|---|
| Platform administrator | Organization portfolio and governance view | All Project names and safe operational metadata (purpose, owner, status, member count, attached-knowledge count, authority gaps, items needing attention), and can open any Project's full workspace directly from the portfolio, member or not | The full-workspace bypass is real, not just metadata visibility -- it exists because `is_project_member`'s RLS already carries an unconditional admin bypass, so the portfolio surfaces it rather than hiding an access the admin already had |
| Platform curator (not a member of the Project) | Organization portfolio and governance view | The same safe operational metadata as admin, plus a "Request membership" action that notes the Project owner | No content bypass -- a curator cannot open a Project's workspace, source titles, documents, artifacts or conversations until an explicit `project_members` row exists |
| Project owner or Project curator (a member) | Project management view | The Projects they govern, their members, approved knowledge attachments, governance configuration and content for which they are authorized | Resource-level restrictions still apply inside the Project |
...
The organization portfolio is an operational overview for both admin and curator, and never exposes source titles, snippets, contents or conversation history through its own metadata query regardless of role. What differs by role is what happens when a viewer follows a row into the actual Project: admin's bypass is the platform's existing, unconditional `is_admin` check inside `is_project_member` (not a separate audited process -- see `docs/dev-request-role-aware-project-views-and-ember-first-workspace.md`'s 2026-09-01 addendum for why an *additional* audited-access process was judged unnecessary here), while a non-member curator has no bypass at all and must request and receive an explicit membership grant first.
