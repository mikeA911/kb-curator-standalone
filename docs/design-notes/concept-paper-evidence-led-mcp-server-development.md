# Concept Paper: Evidence-Led Development of an MCP Server for an Existing Application

**Status:** Concept for development discussion  
**Audience:** Product managers, architects, developers, security reviewers, operations teams, and subject-matter experts  
**Purpose:** Define a reliable process for exposing the capabilities of an existing application through a Model Context Protocol (MCP) server without assuming that its OpenAPI specification represents the whole business system.
...
|---|---|
| UI enhancement | The new interface can perform the selected user activities while preserving existing behavior and permissions. |
...
| Legacy modernization | Existing behavior is either preserved or deliberately retired, with each decision documented. |
...
- behavior that may be changed or replaced;
...
## 3. OpenAPI is evidence, not the complete requirements set
...
- the rationale for existing behavior.
...
## 4. Discover the business system
...
## 5. Trace capabilities to implementation and decisions
...
## 6. Design MCP tools around business intentions
...
## 7. Classify the permitted level of agency
...
## 8. Identity, authorization, and accountability
...
The MCP server should use upstream APIs wherever possible so that established business and authorization controls remain authoritative. Direct database access should not be introduced merely to work around missing API operations; doing so can bypass validation, permissions, events, and audit behavior.
...
- periodic review of tools, permissions, and model behavior.
...
## 10. Evaluation and parity testing

Testing must demonstrate business behavior, not merely successful API connectivity.
...
- reversal or recovery behavior; and
...
Human review remains necessary for business-critical scenarios, especially where legacy behavior is poorly documented.
...
The legacy interface should remain available until capability parity and operational confidence are demonstrated for the intended scope. Incremental replacement reduces the risk of treating undocumented behavior as expendable.

## 12. Required development artefacts
...
These artefacts should be versioned alongside the MCP implementation because application behavior, APIs, risks, and tool boundaries will continue to evolve.
...
## 14. Questions for the development team
...
10. What tests would convince the application owner that existing behavior has been preserved?
...
OpenAPI should be treated as a strong technical input within a larger evidence-led process. The project must begin with intent, discover business capabilities, trace them to authoritative behavior, make explicit exposure decisions, preserve deterministic controls, and validate outcomes incrementally.
