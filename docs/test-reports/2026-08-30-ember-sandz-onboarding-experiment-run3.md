...
- **The staff portfolio correctly reflects everything created.** Logging in as `test-admin` (not a member of any Sandz project) and opening `/projects/portfolio` showed all four new Sandz projects with correct member counts and, correctly, "Membership required" rather than a link into any of them. **Superseded 2026-09-01:** this was the behavior at the time of this run, but per owner direction a non-member admin now gets "Open workspace" unconditionally (they already had the underlying RLS access; the portfolio was just hiding the link). Only a non-member curator still sees a gate here today, and it's now an actionable "Request membership" button rather than a dead label. See `docs/ember/KB-SANDBOX-CAPABILITY-AND-NAVIGATION-CATALOGUE.md`.
